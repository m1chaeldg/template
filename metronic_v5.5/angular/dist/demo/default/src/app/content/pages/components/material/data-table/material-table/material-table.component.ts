...
export class MaterialTableComponent implements OnInit, AfterViewInit {
	...
	displayedColumns3 = ['created', 'state', 'number', 'title'];
	displayedColumns4 = ['id', 'name', 'progress', 'color'];
	...
	dataSource5 = new MatTableDataSource<Element>(ELEMENT_DATA);
	...
	applyFilter2(filterValue: string) {
		filterValue = filterValue.trim(); // Remove whitespace
		filterValue = filterValue.toLowerCase(); // MatTableDataSource defaults to lowercase matches
		this.dataSource7.filter = filterValue;
	}
	...
	ngAfterViewInit() {
    	...
    	this.dataSource7.sort = this.sort2;
	}
		...
		merge(this.sort2.sortChange, this.paginator2.page)
			.pipe(
				...
				switchMap(() => {
					...
					return this.exampleDatabase!.getRepoIssues(
						this.sort2.active, this.sort2.direction, this.paginator2.pageIndex);
				}),
				map(data => {
					// Flip flag to show that loading has finished.
					this.isLoadingResults = false;
			...
			).subscribe(data => this.dataSource6.data = data);
	}
